...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if a_node.prenodes.issubset(self.s_levels[level]):
                actions.add(a_node)
                # Add action node to the list of action nodes. Make nodes child and parents
                for s_node in self.s_levels[level]:
                    s_node.children.add(a_node)
                    a_node.parents.add(s_node)
        # action level from action nodes
    ...
    def add_literal_level(self, level):
        ...
        for a_node in self.a_levels[level - 1]:
            # Add node to the list of state nodes. Make nodes child and parents
            for s_node in a_node.effnodes:
                literals.add(s_node)
                s_node.parents.add(a_node)
                a_node.children.add(s_node)
        # state level from effect nodes
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for pos_effect in node_a1.action.effect_add:
            for neg_effect in node_a2.action.precond_neg:
                return True
        for neg_effect in node_a1.action.effect_rem:
            for pos_effect in node_a2.action.precond_pos:
                return True
        for pos_effect in node_a2.action.effect_add:
            for neg_effect in node_a1.action.precond_neg:
                return True
        for pos_effect in node_a2.action.effect_rem:
            for neg_effect in node_a1.action.precond_pos:
                return True
        return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # symbol is equal, the signs are opposite
